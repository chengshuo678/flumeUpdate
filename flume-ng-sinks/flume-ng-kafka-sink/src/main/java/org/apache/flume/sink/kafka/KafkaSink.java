...
public class KafkaSink extends AbstractSink implements Configurable {
  ...
  private byte[] serializeEvent(Event event, boolean useAvroEventFormat) throws IOException {
    ...
    if (useAvroEventFormat) {
      ...
      if (!writer.isPresent()) {
        writer = Optional.of(new SpecificDatumWriter<AvroFlumeEventNew>(AvroFlumeEventNew.class));
      }
      tempOutStream.get().reset();
      AvroFlumeEventNew e = new AvroFlumeEventNew(toCharSeqMap(event.getHeaders()),
