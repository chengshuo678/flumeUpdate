...
public class KafkaChannel extends BasicChannelSemantics {
  ...
  */
  ...
  private class KafkaTransaction extends BasicTransactionSemantics {
    ...
    private Optional<SpecificDatumWriter<AvroFlumeEventNew>> writer =
            Optional.absent();
    private Optional<SpecificDatumReader<AvroFlumeEventNew>> reader =
            Optional.absent();
      ...
      if (parseAsFlumeEvent) {
        ...
        if (!writer.isPresent()) {
          writer = Optional.of(new
                  SpecificDatumWriter<AvroFlumeEventNew>(AvroFlumeEventNew.class));
        }
        tempOutStream.get().reset();
        AvroFlumeEventNew e = new AvroFlumeEventNew(
    ...
    private Event deserializeValue(byte[] value, boolean parseAsFlumeEvent) throws IOException {
      ...
      if (parseAsFlumeEvent) {
        ...
        if (!reader.isPresent()) {
          reader = Optional.of(
                  new SpecificDatumReader<AvroFlumeEventNew>(AvroFlumeEventNew.class));
        }
        AvroFlumeEventNew event = reader.get().read(null, decoder);
