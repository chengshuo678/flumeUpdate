...
public class DurablePositionTracker implements PositionTracker {
  ...
  private final DataFileWriter<TransferStateFileMetaNew> writer;
  private final DataFileReader<TransferStateFileMetaNew> reader;
  private final TransferStateFileMetaNew metaCache;
  ...
  DurablePositionTracker(File trackerFile, String target)
    ...
    DatumWriter<TransferStateFileMetaNew> dout =
        new SpecificDatumWriter<TransferStateFileMetaNew>(
            TransferStateFileMetaNew.SCHEMA$);

    DatumReader<TransferStateFileMetaNew> din =
        new SpecificDatumReader<TransferStateFileMetaNew>(
            TransferStateFileMetaNew.SCHEMA$);

    writer = new DataFileWriter<TransferStateFileMetaNew>(dout);

    if (trackerFile.exists()) {
      // open it for append
      writer.appendTo(trackerFile);

      reader = new DataFileReader<TransferStateFileMetaNew>(trackerFile, din);
      this.target = reader.getMetaString("file");
    } else {
      // create the file
      this.target = target;
      writer.setMeta("file", target);
      writer.create(TransferStateFileMetaNew.SCHEMA$, trackerFile);
      reader = new DataFileReader<TransferStateFileMetaNew>(trackerFile, din);
    }
    ...
    // initialize @ line = 0;
    metaCache = TransferStateFileMetaNew.newBuilder().setOffset(0L).build();
