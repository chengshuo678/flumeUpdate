...
public class TestKafkaSource {
  ...
  public void testAvroEvent() throws InterruptedException, EventDeliveryException, IOException {
    SpecificDatumWriter<AvroFlumeEventNew> writer;
    ...
    writer = new SpecificDatumWriter<AvroFlumeEventNew>(AvroFlumeEventNew.class);

    Map<CharSequence, CharSequence> headers = new HashMap<CharSequence,CharSequence>();
    headers.put("header1", "value1");
    headers.put("header2", "value2");

    AvroFlumeEventNew e = new AvroFlumeEventNew(headers, ByteBuffer.wrap("hello, world".getBytes()));
    ...
    e = new AvroFlumeEventNew(headers, ByteBuffer.wrap("hello, world2".getBytes()));
    tempOutStream.reset();
    encoder = EncoderFactory.get().directBinaryEncoder(tempOutStream, null);
    writer.write(e, encoder);
    encoder.flush();
