...
public class NettyAvroRpcClient extends AbstractRpcClient implements RpcClient {
  ...
  private AvroSourceProtocolNew.Callback avroClient;
  ...
  private void connect(long timeout, TimeUnit tu) throws FlumeException {
    ...
    try {
      ...
      transceiver = new NettyTransceiver(this.address,
              ...
              tu.toMillis(timeout));
      avroClient =
              SpecificRequestor.getClient(AvroSourceProtocolNew.Callback.class,
                      transceiver);
    } catch (Throwable t) {
  ...
  private void append(Event event, long timeout, TimeUnit tu)
    ...
    final CallFuture<StatusNew> callFuture = new CallFuture<StatusNew>();

    final AvroFlumeEventNew avroEvent = new AvroFlumeEventNew();
  ...
  private void appendBatch(List<Event> events, long timeout, TimeUnit tu)
    ...
    final List<AvroFlumeEventNew> avroEvents = new LinkedList<AvroFlumeEventNew>();
    ...
    while (iter.hasNext()) {
      ...
      for (int i = 0; i < batchSize && iter.hasNext(); i++) {
        ...
        AvroFlumeEventNew avroEvent = new AvroFlumeEventNew();
        avroEvent.setBody(ByteBuffer.wrap(event.getBody()));
        avroEvent.setHeaders(toCharSeqMap(event.getHeaders()));
  ...
  /**
   * Helper method that waits for a StatusNew future to come back and validates
   * that it returns StatusNew == OK.
   * @param callFuture Future to wait on
   * @param timeout Time to wait before failing
   * @param tu Time Unit of {@code timeout}
   * @throws EventDeliveryException If there is a timeout or if StatusNew != OK
   */
  private void waitForStatusOK(CallFuture<StatusNew> callFuture,
                               long timeout, TimeUnit tu) throws EventDeliveryException {
    try {
      StatusNew status = callFuture.get(timeout, tu);
      if (status != StatusNew.OK) {
